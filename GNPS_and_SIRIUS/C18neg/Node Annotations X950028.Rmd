...
Filter for molecular network X950028:
```{r}
node_annotations <- node_annotations %>% 
  filter(componentindex == 28)
...
```{r}
wordcloud(word_freq$Words, word_freq$Freq, min.freq = 1, scale = c(4, 1))
```
...
Plot molecular formula distribution (C > 35):
```{r}
formula_distribution %>% 
  filter(C > 35) %>% 
  pivot_longer(all_of(c("C", "H", "N", "O")), names_to = "Elements", values_to = "Numbers") %>% 
...
Plot molecular formula distribution (C > 35 & O == 9):
```{r}
formula_distribution %>% 
  filter(C > 35 & O == 9) %>% 
  pivot_longer(all_of(c("C", "H", "N", "O")), names_to = "Elements", values_to = "Numbers") %>% 
...
Consensus formula (C > 35):
```{r}
consensus_formula <- formula_distribution %>% 
  filter(C > 35 & O == 9 & H == 83) %>% 
  pivot_longer(all_of(c("C", "H", "N", "O")), names_to = "Elements", values_to = "Numbers") %>% 
...
Distribution of CANOPUS masses (C > 35):
```{r}
formula_distribution %>% 
  filter(C > 35) %>% 
  pivot_longer(all_of(c("C", "H", "N", "O")), names_to = "Elements", values_to = "Numbers") %>% 
