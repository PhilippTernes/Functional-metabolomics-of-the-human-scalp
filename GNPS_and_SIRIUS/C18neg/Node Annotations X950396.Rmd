...
```{r}
node_annotations <- read_delim("mmvec sebumeter.csv")
```

Filter for molecular network X950396:
```{r}
node_annotations <- node_annotations %>% 
  filter(componentindex == 396)

node_annotations %>% nrow()
```
...
```{r}
wordcloud(word_freq$Words, word_freq$Freq, min.freq = 1, scale = c(1.5, 0.5))
```
...
formula_distribution <- node_annotations %>% 
  filter(CAN_molecularFormula != "C25H52ClNO6") %>% 
  mutate(
...
Plot molecular formula distribution (N == 1 & O == 7):
```{r}
formula_distribution %>% 
  filter(N == 1 & O == 7) %>% 
  pivot_longer(all_of(c("C", "H", "N", "O")), names_to = "Elements", values_to = "Numbers") %>% 
...
Plot molecular formula distribution (N == 1 & O == 9):
```{r}
formula_distribution %>% 
  filter(N == 1 & O == 9) %>% 
  pivot_longer(all_of(c("C", "H", "N", "O")), names_to = "Elements", values_to = "Numbers") %>% 
...
Distribution of CANOPUS masses (N == 1 & O == 7):
```{r}
formula_distribution %>% 
  filter(N == 1 & O == 7) %>% 
  pivot_longer(all_of(c("C", "H", "N", "O")), names_to = "Elements", values_to = "Numbers") %>% 
...
Distribution of CANOPUS masses (N == 1 & O == 9):
```{r}
formula_distribution %>% 
  filter(N == 1 & O == 9) %>% 
  pivot_longer(all_of(c("C", "H", "N", "O")), names_to = "Elements", values_to = "Numbers") %>% 
