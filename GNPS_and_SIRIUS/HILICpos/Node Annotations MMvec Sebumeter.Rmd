...
* These predicted molecular formulas are different from the Canopus ones.
* Most of the Zodias predictions contain no N and have high mass error.
* The Canopus ones make more sense and match better the class predictions.
...
Plot molecular formula distribution (N == 1 & C > 20):
```{r}
formula_distribution %>% 
  filter(N == 1 & C > 20) %>% 
  pivot_longer(all_of(c("C", "H", "N", "O")), names_to = "Elements", values_to = "Numbers") %>% 
...
Distribution of CANOPUS masses (N == 1 & C > 20):
```{r}
formula_distribution %>% 
  filter(N == 1 & C > 20) %>% 
  pivot_longer(all_of(c("C", "H", "N", "O")), names_to = "Elements", values_to = "Numbers") %>% 
